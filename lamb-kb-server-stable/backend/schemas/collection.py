...
class EmbeddingsModel(BaseModel):
    ...
    vendor: str = Field(
        ...,
        description="Vendor of the embeddings model (e.g., 'ollama', 'local', 'openai')",
    )
    api_endpoint: Optional[str] = Field(None, description="Custom API endpoint URL")
    apikey: Optional[str] = Field(
        None, description="API key for the endpoint if required"
    )
...
class EmbeddingsModelSafe(BaseModel):
    ...
    api_endpoint: Optional[str] = Field(None, description="Custom API endpoint URL")
    apikey_configured: bool = Field(
        False, description="Whether an API key is configured (key value hidden)"
    )
...
class CollectionBase(BaseModel):
    ...
    name: str = Field(
        ..., description="Name of the collection", min_length=1, max_length=255
    )
    description: Optional[str] = Field(
        None, description="Optional description of the collection"
    )
    visibility: str = Field(
        "private", description="Visibility setting ('private' or 'public')"
    )
    graph_enabled: bool = Field(
        False, description="Whether Graph RAG indexing is enabled for this collection"
    )
...
class CollectionCreate(CollectionBase):
    ...
    embeddings_model: Optional[EmbeddingsModel] = Field(
        None, description="Optional custom embeddings model configuration"
    )
...
class CollectionUpdate(BaseModel):
    ...
    name: Optional[str] = Field(
        None, description="New name of the collection", min_length=1, max_length=255
    )
    description: Optional[str] = Field(
        None, description="New description of the collection"
    )
    visibility: Optional[str] = Field(
        None, description="New visibility setting ('private' or 'public')"
    )
    graph_enabled: Optional[bool] = Field(
        None, description="Enable or disable Graph RAG indexing for this collection"
    )
    embeddings_model: Optional[EmbeddingsModel] = Field(
        None, description="New embeddings model configuration"
    )
...
class CollectionResponse(CollectionBase):
    ...
    embeddings_model: EmbeddingsModelSafe = Field(
        ..., description="Embeddings model configuration (API key hidden)"
    )
    graph_enabled: bool = Field(
        False, description="Whether Graph RAG indexing is enabled for this collection"
    )
...
class CollectionCreateResponse(BaseModel):
    ...
    id: int = Field(
        ..., description="Unique identifier of the newly created collection"
    )
...
class EmbeddingsModelPartial(BaseModel):
    ...
    model: Optional[str] = Field(
        None, description="Name or path of the embeddings model"
    )
...
class BulkUpdateEmbeddingsResponse(BaseModel):
    ...
    collections: List[Dict[str, Any]] = Field(
        ..., description="List of updated collection IDs and names"
    )
