...
class CollectionsService:
        ...
        if hasattr(collection, "__dict__"):
            ...
            coll_dict = {
                ...
                "visibility": (
                    ...
                    if hasattr(collection.visibility, "value")
                    else str(collection.visibility)
                ),
                "owner": collection.owner,
                "creation_date": collection.creation_date,
                "graph_enabled": bool(getattr(collection, "graph_enabled", False)),
                "embeddings_model": CollectionsService._sanitize_embeddings_model(
            ...
            coll_dict["embeddings_model"] = (
                CollectionsService._sanitize_embeddings_model(
    ...
    def _categorize_embedding_error(
        error: Exception, vendor: str, model: str, api_endpoint: str, config_source: str
    ) -> str:
        """
    ...
    def validate_embeddings_config(
        embeddings_model: Dict[str, Any], is_custom_config: bool = False
        ...
        config_source = (
            "custom configuration" if is_custom_config else "environment defaults"
        )
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail=detailed_error
            )
        finally:
            if timeout_supported:
                # Cancel the alarm
    ...
    def create_collection(
        ...
        try:
            ...
            if collection.embeddings_model:
                ...
                print("INFO: [create_collection] Validating embeddings configuration")
                print(
                    f"INFO:   Config source: {'Custom' if is_custom_config else 'Environment defaults'}"
                )
                print(
                    f"INFO:   Vendor: {model_info.get('vendor')}, Model: {model_info.get('model')}"
                )
                ...
                success, error_msg, dimensions = (
                    CollectionsService.validate_embeddings_config(
                        embeddings_model=model_info, is_custom_config=is_custom_config
                    )
                )

                print(
                    f"INFO: [create_collection] Validation successful, embedding dimensions: {dimensions}"
                )
                print(
                    "INFO: [create_collection] Collection will be created with immutable embeddings function"
                )
            ...
            if isinstance(db_collection.embeddings_model, str):
                try:
                    db_collection.embeddings_model = json.loads(
    ...
    def list_collections(
        ...
        return {"total": total, "items": sanitized_collections}

    @staticmethod
    def get_collection(collection_id: int, db: Session) -> Dict[str, Any]:
    ...
    @staticmethod
    def list_files(
        collection_id: int, db: Session, status: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        ...
        # Query files
        query = db.query(FileRegistry).filter(
    ...
    @staticmethod
    def update_file_status(file_id: int, status: str, db: Session) -> Dict[str, Any]:
    ...
    def get_file_content(file_id: int, db: Session) -> Dict[str, Any]:
        ...
        collection = (
            db.query(Collection)
            .filter(Collection.id == file_registry.collection_id)
            .first()
        )
        if not collection:
            raise HTTPException(
        ...
        results = chroma_collection.get(
            where={"filename": source}, include=["documents", "metadatas"]
        )
        ...
        if not results["documents"]:
            raise HTTPException(
                status_code=404, detail=f"No content found for file: {source}"
            )
        ...
        return {
            ...
            "timestamp": (
                file_registry.updated_at.isoformat()
                if file_registry.updated_at
                else None
            ),
        }
    ...
    @staticmethod
    def delete_file(
        collection_id: int, file_id: int, db: Session, hard_delete: bool = True
    ) -> Dict[str, Any]:
        ...
        file_hash = (
            Path(file_url).stem if file_url else Path(file_registry.file_path).stem
        )

        embedding_ids = CollectionsService._find_embedding_ids_for_file(
            chroma_collection, file_hash
        )
    ...
    @staticmethod
    def delete_collection(collection_id: int, db: Session) -> Dict[str, Any]:
        ...
        from database.models import (
            ...
            FileRegistry as DBFileRegistry,
        )

        collection = (
            db.query(DBCollection).filter(DBCollection.id == collection_id).first()
        )
        if not collection:
            raise HTTPException(
                status_code=404, detail=f"Collection with ID {collection_id} not found"
        ...
        removed_files: List[str] = []
        file_entries = (
            db.query(DBFileRegistry)
            .filter(DBFileRegistry.collection_id == collection_id)
            .all()
        )
    ...
    def bulk_update_embeddings_apikey(
        db: Session, owner: str, apikey: str
    ) -> Dict[str, Any]:
        ...
        return DBCollectionService.bulk_update_embeddings_apikey(
            db=db, owner=owner, apikey=apikey
        )
