...
class IngestionService:
    ...
    STATIC_DIR = (
        Path(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) / "static"
    )
    ...
    def save_uploaded_file(
        cls, file: UploadFile, owner: str, collection_name: str
    ) -> Dict[str, str]:
        ...
        """
        print(
            f"DEBUG: [save_uploaded_file] Starting to save file: {file.filename if file else 'None'}"
        )
        cls._ensure_dirs()
        ...
        print(
            f"DEBUG: [save_uploaded_file] Generated unique filename: {unique_filename}"
        )
        ...
        try:
            # Save the file
            print("DEBUG: [save_uploaded_file] Starting file copy operation")
            with open(file_path, "wb") as f:
                # Read in chunks to avoid memory issues with large files
                print("DEBUG: [save_uploaded_file] Reading file content")
                content = file.file.read()
                print(
                    f"DEBUG: [save_uploaded_file] File content read, size: {len(content)} bytes"
                )
                f.write(content)
            print("DEBUG: [save_uploaded_file] File saved successfully")
            ...
            print("DEBUG: [save_uploaded_file] File saved, returning result")
            return result
    ...
    def update_file_status(
        cls, db: Session, file_id: int, status: FileStatus
    ) -> FileRegistry:
        ...
        """
        file_registry = (
            db.query(FileRegistry).filter(FileRegistry.id == file_id).first()
        )
        if file_registry:
    ...
    @classmethod
    def ingest_file(
        cls, file_path: str, plugin_name: str, plugin_params: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        if not plugin:
            ...
            raise HTTPException(
                status_code=404, detail=f"Ingestion plugin '{plugin_name}' not found"
            )
        ...
        try:
            ...
            print("DEBUG: [ingest_file] Calling plugin.ingest()")
            ...
            raise HTTPException(
                status_code=400, detail=f"Failed to ingest file: {str(e)}"
            )

    @classmethod
    def _index_documents_for_kg_rag(
        cls,
        db: Session,
        db_collection: Union[Dict[str, Any], Collection],
        ids: List[str],
        texts: List[str],
        ...
        try:
            ...
            if file_registry_id:
                file_registry = (
                    ...
                    .filter(FileRegistry.id == file_registry_id)
                    .first()
                )

            filename = "unknown"
            if file_registry is not None:
                filename = file_registry.original_filename or filename
            elif metadatas:
                filename = (
                    metadatas[0].get("original_filename")
                    or metadatas[0].get("filename")
                    or metadatas[0].get("source")
                    or filename
                )
            ...
            for index, chunk_id in enumerate(ids):
                ...
                metadata.setdefault(
                    "source_label",
                    metadata.get("section_title") or metadata.get("source") or filename,
                )
                text = texts[index]
                chunks.append(
                    TextChunk(
                        chunk_id=metadata.get("document_id") or chunk_id,
                        text=text,
                        parent_text=metadata.get("parent_text") or text,
                        metadata=metadata,
                    )
                )

            if file_registry is not None:
                file_registry.progress_message = "Extracting graph concepts..."
                file_registry.updated_at = datetime.utcnow()
                db.commit()

            extractor = ConceptExtractor(kg_config=kg_config)
            extraction = extractor.extract_for_chunks(chunks)
            if not extraction.all_concepts and not extraction.relationships:
                return {
                    "enabled": True,
                    "indexed": False,
            ...
            if file_registry is not None:
                file_registry.progress_message = "Writing knowledge graph..."
                file_registry.updated_at = datetime.utcnow()
                db.commit()

            graph_writes = graph_store.ingest_chunks(
            ...
            return {
                "enabled": True,
                "indexed": graph_writes > 0,
                "chunks": len(chunks),
                "concepts": len(extraction.entities),
                "relationships": len(extraction.relationships),
                "graph_writes": graph_writes,
            }
        except Exception as e:
            print(f"WARNING: [kg_rag] Graph indexing skipped after error: {str(e)}")
    ...
    def add_documents_to_collection(
        ...
        """
        print(
            f"DEBUG: [add_documents_to_collection] Starting for collection_id: {collection_id}"
        )
        print(
            f"DEBUG: [add_documents_to_collection] Number of documents: {len(documents)}"
        )

        # Get the collection
        print("DEBUG: [add_documents_to_collection] Getting collection from database")
        db_collection = CollectionService.get_collection(db, collection_id)
        if not db_collection:
            print("DEBUG: [add_documents_to_collection] ERROR: Collection not found")
            raise HTTPException(
        ...
        collection_name = (
            db_collection["name"]
            if isinstance(db_collection, dict)
            else db_collection.name
        )

        print(
            f"DEBUG: [add_documents_to_collection] Found collection: {collection_name}"
        )
        ...
        embedding_config = (
            ...
            if isinstance(db_collection, dict)
            and isinstance(db_collection["embeddings_model"], str)
            else (
                db_collection.embeddings_model
                if not isinstance(db_collection, dict)
                else db_collection["embeddings_model"]
            )
        )
        print(
            f"DEBUG: [add_documents_to_collection] Embedding config: {embedding_config}"
        )
        ...
        print(
            f"DEBUG: [add_documents_to_collection] Using embeddings - vendor: {vendor}, model: {model_name}"
        )

        # Get ChromaDB client
        print("DEBUG: [add_documents_to_collection] Getting ChromaDB client")
        ...
        try:
            print(
                "DEBUG: [add_documents_to_collection] Creating embedding function from collection record"
            )
            ...
            print(
                f"DEBUG: [add_documents_to_collection] Created embedding function: {collection_embedding_function is not None}"
            )

            # Test the embedding function to verify it works
            test_result = collection_embedding_function(
                ["Test embedding function verification"]
            )
            print(
                f"DEBUG: [add_documents_to_collection] Embedding function test successful, dimensions: {len(test_result[0])}"
            )
        except Exception as ef_e:
            print(
                f"DEBUG: [add_documents_to_collection] ERROR creating embedding function: {str(ef_e)}"
            )
            import traceback

            print(
                f"DEBUG: [add_documents_to_collection] Stack trace:\n{traceback.format_exc()}"
            )
            raise HTTPException(
                status_code=500,
        ...
        try:
            print(
                f"DEBUG: [add_documents_to_collection] Verifying ChromaDB collection exists: {collection_name}"
            )
            ...
            if collections and isinstance(collections[0], str):
                ...
                print(
                    "DEBUG: [add_documents_to_collection] Using ChromaDB v0.6.0+ API: collections are strings"
                )
            else:
                ...
                try:
                    ...
                    print(
                        "DEBUG: [add_documents_to_collection] Using older ChromaDB API: collections have name attribute"
                    )
                    ...
                    try:
                        ...
                        print(
                            "DEBUG: [add_documents_to_collection] Verified collection exists by get_collection"
                        )
                    except Exception:
            ...
            if not collection_exists:
                print(
                    f"ERROR: [add_documents_to_collection] ChromaDB collection does not exist: {collection_name}"
                )
                raise HTTPException(
                    status_code=500,
            ...
            print(
                "DEBUG: [add_documents_to_collection] ChromaDB collection retrieved successfully"
            )
            ...
            if existing_metadata:
                ...
                existing_embeddings_model = existing_metadata.get(
                    "embeddings_model", "{}"
                )
                try:
                    ...
                    if existing_vendor != vendor or existing_model != model_name:
                        print(
                            "WARNING: [add_documents_to_collection] Embedding model mismatch detected!"
                        )
                        print(
                            f"WARNING: ChromaDB collection uses - vendor: {existing_vendor}, model: {existing_model}"
                        )
                        print(
                            f"WARNING: SQLite record specifies - vendor: {vendor}, model: {model_name}"
                        )
                        print(
                            "WARNING: Will use the embedding function from SQLite record"
                        )
                        # We continue with the SQLite embedding function but log the warning
                except (json.JSONDecodeError, AttributeError) as e:
                    print(
                        f"WARNING: [add_documents_to_collection] Could not parse embedding model from metadata: {str(e)}"
                    )
            ...
            print(
                f"DEBUG: [add_documents_to_collection] ERROR with ChromaDB collection: {str(e)}"
            )
            import traceback
        ...
        print("DEBUG: [add_documents_to_collection] Preparing documents for ChromaDB")
        ids = []
        texts = []
        metadatas = []
        ...
        try:
            print("DEBUG: [add_documents_to_collection] Adding documents to ChromaDB")
            if len(texts) > 0:
                print(
                    f"DEBUG: [add_documents_to_collection] First document sample: {texts[0][:100]}..."
                )
            ...
            for i in range(0, len(ids), batch_size):
                ...
                print(
                    f"DEBUG: [add_documents_to_collection] Processing batch {i//batch_size + 1}/{(len(ids) + batch_size - 1)//batch_size}"
                )
                ...
                print(
                    f"DEBUG: [add_documents_to_collection] Batch {i//batch_size + 1} completed in {batch_end_time - batch_start_time:.2f} seconds"
                )
                ...
                if file_registry_id:
                    ...
                    if file_reg:
                        ...
                        file_reg.progress_message = (
                            f"Adding chunks to collection... ({batch_end}/{total_docs})"
                        )
                        file_reg.updated_at = datetime.utcnow()
                        db.commit()
                        print(
                            f"DEBUG: [add_documents_to_collection] Updated progress: {batch_end}/{total_docs}"
                        )

            end_time = time.time()
            print(
                f"DEBUG: [add_documents_to_collection] ChromaDB add operation completed in {end_time - start_time:.2f} seconds"
            )

            kg_rag_result = cls._index_documents_for_kg_rag(
                db=db,
            ...
            return {
                ...
                "success": True,
                "embedding_info": {"vendor": vendor, "model": model_name},
                "kg_rag": kg_rag_result,
            }
        except Exception as e:
            print(
                f"DEBUG: [add_documents_to_collection] ERROR adding documents to ChromaDB: {str(e)}"
            )
            import traceback
            ...
            if "api_key" in error_message.lower() or "apikey" in error_message.lower():
                print(
                    "DEBUG: [add_documents_to_collection] Likely an API key issue with embeddings"
                )
                raise HTTPException(
                    status_code=500,
                    detail="Failed to add documents: API key issue with embeddings provider. Check your API key configuration.",
                )
            elif "timeout" in error_message.lower():
                print("DEBUG: [add_documents_to_collection] Request timeout detected")
                raise HTTPException(
                    status_code=500,
                    detail="Failed to add documents: Timeout when generating embeddings. The embedding service may be unavailable.",
                )
            else:
                raise HTTPException(
                    status_code=500,
