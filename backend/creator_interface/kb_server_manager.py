...
logger.setLevel(
    os.getenv("KB_LOG_LEVEL", os.getenv("GLOBAL_LOG_LEVEL", "WARNING")).upper()
)
...
class KBServerManager:
    ...
    def _get_kb_config_for_user(self, creator_user: Dict[str, Any]) -> Dict[str, str]:
        ...
        try:
            ...
            if kb_config and kb_config.get("server_url"):
                ...
                logger.info(
                    f"Using organization-specific KB config for user {user_email} (org: {org_name})"
                )
                api_token = kb_config.get("api_token")
                if not api_token:
                    api_token = self.global_kb_server_token
                return {"url": kb_config.get("server_url"), "token": api_token}
            else:
                logger.info(
                    f"No organization KB config for {user_email}, using global config"
                )

        except Exception as e:
            logger.warning(
                f"Error resolving organization KB config for {user_email}: {e}, falling back to global"
            )

        # Fallback to global environment variables
        if not self.global_kb_server_url:
            raise ValueError("LAMB_KB_SERVER environment variable is required")
        return {"url": self.global_kb_server_url, "token": self.global_kb_server_token}
    ...
    async def is_kb_server_available(self, creator_user: Dict[str, Any] = None):
        ...
        try:
            ...
            logger.warning(
                f"KB server connectivity check failed (url={kb_server_url}): {str(e)}"
            )
            return False
    ...
    async def get_user_knowledge_bases(
        self, creator_user: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for kb in owned_kbs:
            ...
            if kb_id and kb_name:
                if not db_manager.get_kb_registry_entry(kb_id):
                    logger.info(
                        f"Auto-registering KB {kb_id} ('{kb_name}') on first access"
                    )
                        ...
                        try:
                            ...
                            if isinstance(kb["creation_date"], str):
                                ...
                                dt = datetime.fromisoformat(
                                    kb["creation_date"].replace("Z", "+00:00")
                                )
                            ...
                            logger.warning(
                                f"Could not parse creation_date from KB server: {e}"
                            )
                            creation_date = None
                    ...
                    if creation_date:
                        ...
                        logger.info(
                            f"Updated created_at for KB {kb_id} to match KB server creation date"
                        )
        ...
        logger.info(
            f"Found {len(kb_registry_entries)} owned KB registry entries for user {user_id}"
        )
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if registry_created_at:
                            ...
                            kb_data["created_at"] = int(time.time())
                        kb_data["updated_at"] = entry.get(
                            "updated_at", int(time.time())
                        )

                        owned_kbs_list.append(kb_data)
                        logger.info(
                            f"Fetched owned KB {kb_id}: is_shared={kb_data['is_shared']}"
                        )

                    elif response.status_code == 404:
                        # Stale entry - KB deleted from KB Server
                        logger.warning(
                            f"Stale registry entry: KB {kb_id} not found in KB Server, removing from registry"
                        )
                        db_manager.delete_kb_registry_entry(kb_id)
                    else:
                        logger.warning(
                            f"KB Server returned {response.status_code} for KB {kb_id}, skipping"
                        )
                    ...
                    if e.response.status_code == 404:
                        # Stale entry cleanup
                        logger.warning(
                            f"Stale registry entry detected: KB {kb_id} not found"
                        )
                        db_manager.delete_kb_registry_entry(kb_id)
                    else:
    ...
    async def get_org_shared_knowledge_bases(
        self, creator_user: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
        ...
        logger.info(
            f"Found {len(kb_registry_entries)} shared KB registry entries for user {user_id}"
        )
            ...
            for entry in kb_registry_entries:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        kb_data["shared_by"] = entry.get("owner_name") or entry.get(
                            "owner_email", "Unknown"
                        )
                        # Use created_at from registry (LAMB database) instead of KB server
                        # If not in registry, fallback to current time
                        ...
                        logger.info(
                            f"Fetched shared KB {kb_id}: shared_by={kb_data['shared_by']}"
                        )
    ...
    async def _fetch_owned_kbs_from_kb_server(
        self, creator_user: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
        ...
        params = {
            "owner": str(
                creator_user.get("id")
            )  # Convert to string as KB server expects string
        }

        kb_server_collections_url = f"{kb_server_url}/collections"
        logger.info(
            f"Requesting user's owned collections from KB server at {kb_server_collections_url}"
        )
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if isinstance(response_data, list):
                        ...
                        and "collections" in response_data
                    ):
                        ...
                        logger.warning(
                            f"Unexpected response format from KB server: {type(response_data)}"
                        )
                        if isinstance(response_data, dict):
                            for key, value in response_data.items():
                                if isinstance(value, list):
                                    ...
                                    logger.info(
                                        f"Using key '{key}' which contains a list of {len(collections_data)} items"
                                    )
                                    break

                    logger.info(
                        f"Found {len(collections_data)} owned KBs from KB Server"
                    )
                    return collections_data

                else:
                    logger.error(
                        f"KB server returned non-200 status: {response.status_code}"
                    )
                    return []
    ...
    async def create_knowledge_base(
        self, kb_data: KnowledgeBaseCreate, creator_user: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if not original_name or not original_name.strip():
            ...
            raise HTTPException(
                status_code=400, detail="Knowledge Base name is required"
            )

        # Sanitize name (lowercase, spaces to underscores, remove special chars)
        sanitized_name, was_modified = sanitize_name(
            original_name, max_length=50, to_lowercase=True
        )
        ...
        async with httpx.AsyncClient() as client:
            kb_server_collections_url = f"{kb_server_url}/collections"
            logger.info(
                f"Creating collection in KB server at {kb_server_collections_url}: {sanitized_name}"
            )
            ...
            try:
                ...
                if hasattr(kb_data, "metadata") and kb_data.metadata:
                    if (
                        isinstance(kb_data.metadata, dict)
                        and "description" in kb_data.metadata
                    ):
                        ...
                        if not description and kb_data.metadata["description"]:
                            ...
                            logger.info(
                                f"Using description from metadata: {description}"
                            )
            ...
            collection_data = {
                ...
                "owner": str(
                    creator_user.get("id")
                ),  # Use ID instead of email for privacy (as string)
            ...
            try:
                ...
                if response.status_code == 201:
                    ...
                    try:
                        ...
                        logger.error(
                            f"Failed to register KB in LAMB registry: {reg_err}"
                        )
                    ...
                    # Handle error
                    logger.error(
                        f"KB server returned non-201 status: {response.status_code}"
                    )
    ...
    async def get_knowledge_base_details(
        ...
        async with httpx.AsyncClient() as client:
            ...
            logger.info(
                f"Requesting collection from KB server at {kb_server_collection_url}"
            )

            try:
                # Send request to KB server
                response = await client.get(
                    kb_server_collection_url, headers=self._get_auth_headers(kb_token)
                )
                logger.info(f"KB server response status: {response.status_code}")
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f"Retrieved collection data from KB server for ID: {kb_id}"
                    )
                    ...
                    if access_type != "shared" and collection_data.get("owner") != str(
                        creator_user.get("id")
                    ):
                        # Check if collection is public
                        if collection_data.get("visibility") != "public":
                            logger.warning(
                                f"User {creator_user.get('email')} (ID: {creator_user.get('id')}) tried to access collection {kb_id} owned by {collection_data.get('owner')}"
                            )
                            raise HTTPException(
                                status_code=403,
                    ...
                    files_response = await client.get(
                        files_url, headers=self._get_auth_headers(kb_token)
                    )
                    ...
                    if files_response.status_code == 200:
                        ...
                        logger.info(
                            f"DEBUG: Raw files data from KB server: {files_data}"
                        )
                        for file in files_data:
                            ...
                            if "created_at" in file:
                                ...
                                if isinstance(file["created_at"], str):
                                    try:
                                        ...
                                        created_at_str = file["created_at"].split(".")[
                                            0
                                        ]
                                        ...
                                        logger.warning(
                                            f"Failed to parse created_at: {e}"
                                        )
                            ...
                            logger.info(
                                f"DEBUG: Mapped file data: id={file_id}, filename={filename}, size={size}"
                            )
                        logger.info(
                            f"Retrieved {len(files)} files for collection {kb_id}"
                        )
                    else:
                        logger.warning(
                            f"Failed to retrieve files for collection {kb_id}, status: {files_response.status_code}"
                        )
                    ...
                    result = {
                        ...
                        "metadata": {
                            "description": collection_data.get("description", ""),
                            "access_control": collection_data.get(
                                "visibility", "private"
                            ),
                            "graph_enabled": bool(
                                collection_data.get("graph_enabled", False)
                            ),
                        },
                        "graph_enabled": bool(
                            collection_data.get("graph_enabled", False)
                        ...
                        "created_at": collection_data.get(
                            "created_at", int(time.time())
                        ),
                    }
    ...
    async def update_knowledge_base(
        self, kb_id: str, kb_data: KnowledgeBaseUpdate, creator_user: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        async with httpx.AsyncClient() as client:
            kb_server_collection_url = f"{kb_server_url}/collections/{kb_id}"
            logger.info(
                f"Connecting to KB server at: {kb_server_collection_url} to update collection"
            )
            ...
            try:
                if hasattr(kb_data, "metadata") and kb_data.metadata:
                    if (
                        ...
                        ) and kb_data.metadata["description"]:
                            description = kb_data.metadata["description"]
                            logger.info(
                                f"Using description from metadata for update: {description}"
                            )
            except Exception as md_err:
                logger.warning(
                    f"Error extracting metadata description during update: {str(md_err)}"
                )
            ...
            try:
                ...
                if get_response.status_code == 200:
                    ...
                    if collection_data.get("owner") != str(creator_user.get("id")):
                        logger.error(
                            f"User {creator_user.get('email')} (ID: {creator_user.get('id')}) is not the owner of knowledge base {kb_id}"
                        )
                        raise HTTPException(
                            status_code=403,
                    ...
                    logger.info(
                        f"KB server update response status: {response.status_code}"
                    )
                    ...
                    if response.status_code == 200:
                        ...
                        logger.info(
                            f"Successfully updated knowledge base {kb_id} in KB server"
                        )
                        ...
                        logger.error(
                            f"Knowledge base with ID {kb_id} not found in KB server"
                        )
                        raise HTTPException(
                    ...
                    logger.error(
                        f"KB server returned non-200 status when getting collection: {get_response.status_code}"
                    )
                    error_detail = "Unknown error"
                    try:
    ...
    async def delete_knowledge_base(
        self, kb_id: str, creator_user: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        logger.info(
            f"Deleting knowledge base {kb_id} for user {creator_user.get('email')}"
        )
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if get_resp.status_code == 404:
                    logger.error(
                        f"Knowledge base with ID {kb_id} not found in KB server (already deleted?)"
                    )
                    raise HTTPException(
                        status_code=404, detail="Knowledge base not found"
                    )
                if get_resp.status_code != 200:
                    ...
                    raise HTTPException(
                        ...
                        detail=f"KB server error fetching collection: {detail}",
                    )
                ...
                if collection_data.get("owner") != owner_id:
                    logger.error(
                        f"Ownership mismatch: user {owner_id} != collection owner {collection_data.get('owner')}"
                    )
                    raise HTTPException(
                        status_code=403,
                ...
                if delete_resp.status_code in (200, 202, 204):
                    ...
                    if isinstance(body, dict):
                        if "deleted_embeddings" in body:
                            response_payload["deleted_embeddings"] = body.get(
                                "deleted_embeddings"
                            )
                        if "removed_files" in body:
                            response_payload["removed_files"] = body.get(
                                "removed_files"
                            )
                        if "name" in body:
                ...
                if delete_resp.status_code == 404:
                    ...
                    logger.warning(
                        "DELETE returned 404 after GET succeeded; treating as already deleted"
                    )
                    return {
                        "kb_id": str(kb_id),
                        "status": "success",
                ...
                raise HTTPException(
                    ...
                    detail=f"KB server deletion error: {detail}",
                )
    ...
    async def query_knowledge_base(
        ...
        logger.info(
            f"Querying knowledge base {kb_id} with: {query_data.get('query_text', '')}"
        )
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                logger.info(
                    f"Checking collection access at KB server: {kb_server_collection_url}"
                )

                headers = self._get_content_type_headers(kb_token)

                get_response = await client.get(
                    kb_server_collection_url, headers=headers
                )
                ...
                if get_response.status_code != 200:
                    if get_response.status_code == 404:
                        ...
                        logger.error(
                            f"KB server returned error status: {get_response.status_code}"
                        )
                        error_detail = "Unknown error"
                        try:
                            error_data = get_response.json()
                ...
                query_response = await client.post(
                    query_url, headers=headers, json=query_payload
                )

                if query_response.status_code == 200:
                    # Successfully queried
                    query_result = query_response.json()
                    logger.info("Query executed successfully against KB server")
                    ...
                    if isinstance(actual_results, list) and actual_results:
                        logger.info(
                            f"Found {len(actual_results)} results in response under 'results' key"
                        )
                    else:
                        logger.warning(
                            f"No results found in query response under 'results' key. Response structure: {list(query_result.keys())}"
                        )
                    ...
                    # Handle query error
                    logger.error(
                        f"KB server returned error status during query: {query_response.status_code}"
                    )
                    error_detail = "Unknown error"
                    try:
    ...
    async def upload_files_to_kb(
        self, kb_id: str, files: List[Any], creator_user: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if collection_data.get("owner") != str(creator_user.get("id")):
                    logger.error(
                        f"User {creator_user.get('email')} is not the owner of knowledge base {kb_id}"
                    )
                    raise HTTPException(
                        status_code=403,
                ...
                if "owner" in collection_data and collection_data["owner"] != str(
                    creator_user.get("id")
                ):
                    logger.info(
                        "Correcting owner field in collection data for KB server"
                    )
                ...
                for file in files:
                    try:
                        ...
                        logger.info(
                            f"Uploading file {file.filename} to KB server for collection {kb_id}"
                        )
                        ...
                        form_data = {
                            "collection_id": str(
                                kb_id
                            ),  # Explicitly include collection_id as string in the form data
                            ...
                            "owner": str(
                                creator_user.get("id")
                            ),  # Explicitly include owner as string
                        }
                        logger.info(
                            f"Including explicit collection_id={kb_id} and plugin params in form data"
                        )
                        ...
                        ingest_url = (
                            f"{kb_server_url}/collections/{str(kb_id)}/ingest-file"
                        )
                        ...
                        curl_form_parts = " ".join(
                            [f"-F '{k}={v}'" for k, v in form_data.items()]
                        )
                        ...
                        async with httpx.AsyncClient(
                            timeout=300.0
                        ) as ingestion_client:  # 5 minutes timeout
                            logger.info(
                                "Sending ingestion request with extended timeout (300s)"
                            )
                        ...
                        if (
                            ...
                            or ingest_response.status_code == 201
                        ):
                            # Successfully uploaded
                            file_data = ingest_response.json()
                            logger.info(
                                f"File {file.filename} ingested successfully to KB server with ID {file_data.get('id')}"
                            )
                            ...
                            # Handle upload error
                            logger.error(
                                f"KB server returned error status during file ingestion: {ingest_response.status_code}"
                            )
                            error_detail = "Unknown error"
                            try:
    ...
    async def delete_file_from_kb(
        self, kb_id: str, file_id: str, creator_user: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        logger.info(
            f"Deleting file {file_id} from knowledge base {kb_id} via KB server"
        )
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                logger.info(
                    f"KB server delete file response status: {delete_response.status_code}"
                )

                if (
                    delete_response.status_code == 200
                    or delete_response.status_code == 204
                ):
                    # Successfully deleted file
                    logger.info(
                        f"Successfully deleted file {file_id} from knowledge base {kb_id}"
                    )
                    ...
                    raise HTTPException(
                        status_code=404, detail="File not found in knowledge base"
                    )
                else:
                    # Handle other errors
                    logger.error(
                        f"KB server returned error status during file deletion: {delete_response.status_code}"
                    )
                    error_detail = "Unknown error"
                    try:
    ...
    async def list_ingestion_jobs(
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    jobs_url, headers=self._get_auth_headers(kb_token), params=params
                )

                if response.status_code == 200:
                    data = response.json()
                    logger.info(
                        f"Retrieved {len(data.get('items', []))} ingestion jobs"
                    )
                    return data
                elif response.status_code == 404:
                    raise HTTPException(
            ...
            raise HTTPException(
                status_code=503, detail=f"Unable to connect to KB server: {e}"
            )

    async def get_ingestion_job_status(
        self, kb_id: str, job_id: int, creator_user: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    job_url, headers=self._get_auth_headers(kb_token)
                )
                ...
                if response.status_code == 200:
                    ...
                    raise HTTPException(
                        status_code=404, detail="Ingestion job not found"
                    )
                else:
                    error_detail = self._extract_error_detail(response)
                    raise HTTPException(
    ...
    async def get_ingestion_status_summary(
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    status_url, headers=self._get_auth_headers(kb_token)
                )

                if response.status_code == 200:
                    data = response.json()
                    logger.info(
                        f"Status summary: {data.get('total_jobs', 0)} total jobs"
                    )
                    return data
                elif response.status_code == 404:
                    raise HTTPException(
    ...
    async def cancel_ingestion_job(
        ...
        cancel_url = (
            f"{kb_server_url}/collections/{kb_id}/ingestion-jobs/{job_id}/cancel"
        )

        try:
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    cancel_url, headers=self._get_auth_headers(kb_token)
                )
    ...
    async def get_ingestion_plugins(self) -> Dict[str, Any]:
        ...
        """
        logger.info(
            "Getting available ingestion plugins from KB server (using global config)"
        )
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f"Successfully retrieved {len(plugins_data)} ingestion plugins"
                    )
                    return plugins_data
                else:
                    logger.error(
                        f"KB server returned error status: {response.status_code}"
                    )
                    error_detail = "Unknown error"
                    try:
                        error_data = response.json()
    ...
    async def plugin_ingest_file(
        ...
        logger.info(
            f"Ingesting file {file.filename} to knowledge base {kb_id} using plugin {plugin_name}"
        )
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                logger.info(
                    f"Using plugin {plugin_name} with parameters: {plugin_params}"
                )
                ...
                if plugin_params:
                    try:
                        ...
                        logger.info(
                            f"Serialized plugin_params: {form_data['plugin_params']}"
                        )
                    except TypeError as json_err:
                        logger.error(
                            f"Could not serialize plugin_params to JSON: {json_err}"
                        )
                ...
                logger.info(
                    f"Sending plugin_name: '{plugin_name}' (type: {type(plugin_name)})"
                )
                ...
                logger.info(
                    f"Final Form data being sent: {form_data}"
                )  # Log final form data
                ...
                if ingest_response.status_code in [200, 201]:
                    ...
                    logger.info(
                        f"File {file.filename} ingestion initiated using plugin {plugin_name}"
                    )
                    ...
                    if response_status == "processing":
                        ...
                        status_message = (
                            f"File successfully ingested using plugin {plugin_name}"
                        )
                    ...
                    result = {
                        ...
                        "file": {
                            "id": str(
                                ingest_data.get(
                                    "id", ingest_data.get("file_registry_id", "unknown")
                                )
                            ),
                            "filename": file.filename,
                            "size": len(content),
                        ...
                        "file_registry_id": ingest_data.get(
                            "file_registry_id", ingest_data.get("id")
                        ),
                        "collection_id": kb_id,
                    }
                    ...
                    if isinstance(ingest_data, dict):
                        if "documents" in ingest_data:
                            result["document_count"] = len(
                                ingest_data.get("documents", [])
                            )
                        if "documents_added" in ingest_data:
                            result["document_count"] = ingest_data.get(
                                "documents_added", 0
                            )
                        ...
                        if "original_filename" in ingest_data:
                            result["original_filename"] = ingest_data[
                                "original_filename"
                            ]
